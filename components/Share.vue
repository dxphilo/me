...
function twitterShare() {
  window.open(
    `https://twitter.com/share?url=${window.location.href}&text=${title} by John Philip`,
    "_blank"
  );
}
function linkedinShare() {
  window.open(
    `https://www.linkedin.com/shareArticle?url=${window.location.href}&title=${title}&summary=${subtitle}&source=${window.location.origin}`,
    "_blank"
  );
}
...
function redditShare() {
  window.open(
    `https://reddit.com/submit?url=${window.location.href}&title=${title}`
  );
}
</script>
<style scoped>
